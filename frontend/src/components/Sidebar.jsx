  ...
  return (
    <aside className="h-full w-20 lg:w-72 border-r border-base-300 flex flex-col transition-all duration-200">
      ...
      <div className="overflow-y-auto w-full py-3">
        ...
        {searchQuery && (
          <div className="mb-4">
            <div className="px-5 text-xs text-zinc-500 font-semibold mb-2 uppercase hidden lg:block">Search Results</div>
            {isSearching ? (
              ...
              searchResults.map((user) => (
                <div key={user._id} className="w-full p-3 flex items-center justify-between hover:bg-base-300 transition-colors">
                  ...
                  {user.requestStatus === 'friend' ? (
                    <span className="text-xs text-green-500 hidden lg:block">Friend</span>
                  ) : user.requestStatus === 'sent' ? (
                    <span className="text-xs text-zinc-500 hidden lg:block">Sent</span>
                  ) : user.requestStatus === 'received' ? (
                    <span className="text-xs text-blue-500 hidden lg:block">Received</span>
                  ) : (
                    <button
        ...
        {!searchQuery && friendRequests.length > 0 && (
          <div className="mb-4 border-b border-base-300 pb-2">
            <div className="px-5 text-xs text-zinc-500 font-semibold mb-2 uppercase hidden lg:block">Friend Requests</div>
        ...
        {!searchQuery && groups.length > 0 && (
          <div className="mb-4">
            <div className="px-5 text-xs text-zinc-500 font-semibold mb-2 uppercase hidden lg:block">Groups</div>
            {groups.map((group) => (
              <button
                key={group._id}
        ...
        {!searchQuery && (
          <>
            <div className="px-5 text-xs text-zinc-500 font-semibold mb-2 uppercase hidden lg:block">Friends</div>
